...
class BettingMiddleItemWidget extends StatelessWidget {
          ...
          child: Column(
            ...
            children: [
              Text(
                ...
                style: TextStyle(
                    fontSize: 13,
                    color: Color(content?.titleColor ?? 0xFF091722),
  ...
  saveColorType(WS.Content con) {
    if (con.type?.contains(GameRuleUtil.GameType_Second) == true) {
      ...
      setTag(GameRuleUtil.Key_Num,
          con.type?.replaceAll("${GameRuleUtil.GameType_Second}_", "") ?? "");
      ...
      setTag(GameRuleUtil.Key_Num,
          con.type?.replaceAll("${GameRuleUtil.GameType_First}_", ""));
      ...
      setTag(GameRuleUtil.Key_Num,
          con.type?.replaceAll("${GameRuleUtil.GameType_Three}_", ""));
    } else {
    ...
    if (keyMap[GameRuleUtil.Key_Num] != null) {
      ...
      if (con.type?.contains(GameRuleUtil.GameType_Big) == true) {
        String betTypeName =
            GameRuleUtil.getBallNameMaxMin(GameRuleUtil.GameType_Big);
        ssb = betTypeName;
      } else if (con.type?.contains(GameRuleUtil.GameType_Small) == true) {
        String betTypeName =
            GameRuleUtil.getBallNameMaxMin(GameRuleUtil.GameType_Small);
        ssb = betTypeName;
      } else if (con.type?.contains(GameRuleUtil.GameType_Odd) == true) {
        String betTypeName =
            GameRuleUtil.getBallNameMaxMin(GameRuleUtil.GameType_Odd);
        ssb = betTypeName;
      } else if (con.type?.contains(GameRuleUtil.GameType_Even) == true) {
        String betTypeName =
            GameRuleUtil.getBallNameMaxMin(GameRuleUtil.GameType_Even);
        ssb = betTypeName;
      } else {
        ssb = con.name ?? "";
      }
