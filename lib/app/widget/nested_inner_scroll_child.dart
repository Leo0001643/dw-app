...
class NestedInnerScrollCoordinator
  ...
  Key? _getInnerScrollViewKey(_NestedScrollPosition position) {
    ...
    if (context is ScrollableState && context.bucket != null) {
  ...
  void updateUserScrollDirection(ScrollDirection value) {
    ...
    for (var element in _innerPositions) {
      if(element.context.notificationContext != null){
  ...
  void applyUserOffset(double delta) {
    ...
    if (!innerScroll || innerPositionList.isEmpty) {
      ...
      // loggerArray(["走这里了吗1",innerScroll,innerPositionList.isEmpty,delta]);
    } else {
      double remainDelta =
      innerPositionList.first.applyClampedDragUpdate(delta);
      // loggerArray(["走这里了吗2",innerScroll,innerPositionList.length,delta]);
    ...
    switch (mode) {
      case _NestedBallisticScrollActivityMode.outer:
        ...
        return _NestedOuterBallisticScrollActivity(
            coordinator, this, metrics, simulation, context.vsync,false);
      case _NestedBallisticScrollActivityMode.inner:
        return _NestedInnerBallisticScrollActivity(
            coordinator, this, simulation, context.vsync,false);
