...
class Gt4FlutterPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        when (call.method) {
            "initWithCaptcha" -> {
                initWithCaptcha(activity!!, call.argument<String>("captchaId").toString(),call.argument<String>("language").toString())
            }
            "verify" -> {
                verifyWithCaptcha()
            }
    ...
    private fun initWithCaptcha(context: Context, captchaId: String, lan: String) {
        print("init called")
        val configBuilder = GTCaptcha4Config.Builder()
        configBuilder.setLanguage(lan)
        gtCaptcha4Client = GTCaptcha4Client.getClient(context)
            .init(captchaId, configBuilder.build())
    }
