...
class DefaultPickerStyle extends PickerStyle {
  DefaultPickerStyle({bool haveRadius = false, String? title}) {
    ...
    if (title != null && title != '') {
      this.title = Center(child: Text(title, style: const TextStyle(color: Colors.grey, fontSize: 14)));
    }
  }
  ...
  DefaultPickerStyle.dark({bool haveRadius: false, String? title}) {
    commitButton = Container(
      ...
      child: const Text('确定', style: TextStyle(color: Colors.white, fontSize: 16.0)),
    );

    cancelButton = Container(
      alignment: Alignment.center,
      padding: const EdgeInsets.only(left: 22, right: 12),
      child: const Text('取消', style: TextStyle(color: Colors.white, fontSize: 16.0)),
    );

    headDecoration = BoxDecoration(
        color: Colors.grey[800],
        borderRadius:
            !haveRadius ? null : const BorderRadius.only(topLeft: Radius.circular(10), topRight: Radius.circular(10)));

    if (title != null && title != '') {
      this.title = Center(child: Text(title, style: const TextStyle(color: Colors.white, fontSize: 14)));
    }
