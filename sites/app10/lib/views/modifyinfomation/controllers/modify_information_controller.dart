...
class ModifyInformationController extends BaseController {
  ...
  final List<String> questionList = ['您的出生地是?','您的父亲姓名是?','您的母亲姓名是?'
    ,'您的配偶姓名是?','您的高中班主任的名字是?','您的初中班主任的名字是?','您的小学班主任的姓名是?','您最新换的一首歌名是?'];
  @override
  void onInit() {
    super.onInit();
  ...
  void onReady() async {
    ...
    inputControllerWeChat.text=_userState.wechat == "0" ? "" : _userState.wechat;
  ...
  Future<void> postData() async {
    ...
    dto.regQq = inputControllerQQ.text == "" ? _userState.qq : inputControllerQQ.text;
    dto.regWeixin =  inputControllerWeChat.text == "" ? _userState.wechat : inputControllerWeChat.text;
    // dto.regPhone =  inputControllerPhone.text == "" ? _userState.phone : inputControllerPhone.text;
