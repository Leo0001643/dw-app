...
class AccountDataModifyController extends ChangeUserinfoLogic {
  ...
  bool _isCanModify = false;

  bool get isCanModify => _isCanModify;

  bool _isModifyPhone = false;

  bool get isModifyPhone => _isModifyPhone;

  bool get isSafeAnswer =>
      _keyword == "safeQuestion" || _keyword == "safeAnswer";
  ...
  void onReady() async {
    ...
    if (_keyword == "qq") {
      ...
      inputController.text =
          _userState.userInfo.regQq! == "0" ? "" : _userState.userInfo.regQq!;
    } else if (_keyword == "wechat") {
      _isCanModify = true;
      _title = "微信号码".tr;
      _label = "微信号码".tr;
      _hintText = "请输入微信号码".tr;
      inputController.text = _userState.userInfo.regWeixin! == "0"
          ? ""
          : _userState.userInfo.regWeixin!;
    } else if (_keyword == "safeQuestion") {
      _title = "安全问题".tr;
      _label = "安全问题".tr;
      _hintText = "如需绑定添加请联系在线客服".tr;
      inputController.text =
          _userState.userInfo.regWen! == "0" ? "" : _userState.userInfo.regWen!;
    } else if (_keyword == "safeTip") {
      _title = "安全提示".tr;
      _label = "安全提示".tr;
      _hintText = "如需绑定添加请联系在线客服".tr;
      inputController.text = _userState.userInfo.regWenT! == "0"
          ? ""
          : _userState.userInfo.regWenT!;
    } else if (_keyword == "safeAnswer") {
      _title = "安全问题回答".tr;
      _label = "安全问题回答".tr;
      _hintText = "如需绑定添加请联系在线客服".tr;
      inputController.text = _userState.userInfo.regWenD! == "0"
          ? ""
          : _userState.userInfo.regWenD!;
    } else if (_keyword == "phone") {
      _title = "手机号".tr;
      _label = "手机号".tr;
      if (_userState.userInfo.regPhone! == "0" ||
          (_userState.userInfo.regPhone?.isEmpty ?? true)) {
        _isCanModify = true;
        _hintText = "请输入手机号".tr;
      } else {
        _isCanModify = false;
        _hintText = "如需添加请联系在线客服".tr;
      }
      inputController.text = _userState.userInfo.regPhone! == "0"
          ? ""
          : _userState.userInfo.regPhone!;
    }
  ...
  Future<void> postData() async {
    ...
    dto.regMobilecode = '';
    if (_keyword == "phone") {
      dto.regPhone = inputController.text;
    } else if (_keyword == "qq") {
