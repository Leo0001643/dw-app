...
class GameCategoryController extends GameCategoryLogic {
  static GameCategoryController get to => Get.find();
  var isNormalListView = true.obs;
  Navconfig navconfigCategory = Navconfig(); //电子、棋牌
...
// 1收藏，0历史
  ...
  final RxInt _selectedMenuIndex = 1.obs;
  int get selectedMenuIndex => _selectedMenuIndex.value;
  set selectedMenuIndex(value) => _selectedMenuIndex.value = value;

  // RxList<SubChildren> leftMenus = RxList<SubChildren>([]);

  @override
  void onInit() {
    super.onInit();
    _argsments.value = Get.arguments;
    loadMyCollect();
  ...
  getGameCategoryData() {
    HomePageController.to.navConfigList.forEach((element) {
      ...
      //
      if (element.subChildren?.isEmpty == true) return;
      if (element.subChildren!.length < 2) return;
      int subType = element.subChildren?[1].type ?? 2;

      if (subType == _argsments.value.type) {
        element.subChildren?.removeWhere((e) => e.ischangshang != 1);
        navconfigCategory = element;

        if (navconfigCategory.subChildren?.first.nameCn != '强档活动') {
          SubChildren sub = SubChildren();
          sub.nameCn = "强档活动";
          navconfigCategory.subChildren?.insert(0, sub);
        }
  ...
  String gainImageUrl(int index) {
    ...
    List<SubChildren>? sub = navconfigCategory.subChildren;
    print('sub == ${sub?.length.toString()}');
  ...
  void refreshNewFactoryTotalData() {
    ...
    SubChildren children = navconfigCategory.subChildren![selectedMenuIndex];
    onSelectProvider(
      children,
    );
