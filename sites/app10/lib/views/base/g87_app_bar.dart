...
class G87AppBar extends StatefulWidget implements PreferredSizeWidget {
  G87AppBar(
    this.title, {
    Key? key,
    this.leading,
  ...
  bool _getEffectiveCenterTitle(ThemeData themeData) {
    ...
    switch (themeData.platform) {
      ...
      case TargetPlatform.iOS:
        return actions == null || (actions?.length ?? 0) < 2;
    }
    return false;
  }

  @override
  _G87AppBarState createState() => _G87AppBarState();
}

class _G87AppBarState extends State<G87AppBar> {
  static const double _defaultElevation = 4.0;
  TextStyle centerStyle = TextStyle();
  ...
  Widget build(BuildContext context) {
    ...
    if (title != null) {
      ...
      title = DefaultTextStyle(
        style: this.centerStyle ?? TextStyle(),
        softWrap: false,
      ...
      appBar = Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(colors: [
            widget.gradientStart ?? Colors.white,
            widget.gradientEnd ?? Colors.white
          ]),
        ),
        child: appBar,
      );
